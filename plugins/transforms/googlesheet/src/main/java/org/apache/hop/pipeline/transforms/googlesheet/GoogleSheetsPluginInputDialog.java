...
public class GoogleSheetsPluginInputDialog extends BaseTransformDialog implements ITransformDialog {
    ...
    public GoogleSheetsPluginInputDialog(Shell parent,IVariables variables, Object in, PipelineMeta pipelineMeta, String name) {
    ...
    public String open() {
        ...
        wlTransformName.setText(BaseMessages.getString(PKG, "GoogleSheetsPluginInputDialog.Stepname.Label"));
	   ...
	   // privatekey - Text
        privateKeyStore = new TextVar(variables,serviceAccountComposite, SWT.SINGLE | SWT.LEFT | SWT.BORDER);
        ...
        // spreadsheetKey - Text
        spreadsheetKey = new TextVar(variables,spreadsheetComposite, SWT.SINGLE | SWT.LEFT | SWT.BORDER);
        ...
        wFields = new TableView(variables, fieldsComposite, SWT.FULL_SELECTION | SWT.MULTI, columnInformation, 1, modifiedListener, props);
        ...
        BaseTransformDialog.positionBottomButtons(shell, new Button[]{wOk, wCancel}, margin, tabFolder);
        ...
        testServiceAccountButton.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                try {						
					...
					Drive service = new Drive.Builder(HTTP_TRANSPORT, JSON_FACTORY, GoogleSheetsPluginCredentials.getCredentialsJson(scope,variables.resolve(privateKeyStore.getText()))).setApplicationName(APPLICATION_NAME).build();
...
//Display worksheets
        worksheetIdButton.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                try {
					...
					Sheets service = new Sheets.Builder(HTTP_TRANSPORT, JSON_FACTORY, GoogleSheetsPluginCredentials.getCredentialsJson(scope,variables.resolve(privateKeyStore.getText()))).setApplicationName(APPLICATION_NAME).build();
					Spreadsheet response1= service.spreadsheets().get(variables.resolve(spreadsheetKey.getText())).setIncludeGridData(false).execute();
    ...
    private void getSpreadsheetFields() {
        try {
			...
			ValueRange result = service.spreadsheets().values().get(variables.resolve(meta.getSpreadsheetKey()), range).execute();
