...
public class GoogleSheetsPluginInput extends BaseTransform<GoogleSheetsPluginInputMeta, GoogleSheetsPluginInputData> implements ITransform<GoogleSheetsPluginInputMeta, GoogleSheetsPluginInputData> {

  private static Class<?> PKG = GoogleSheetsPluginInput.class; // for i18n purposes, needed by Translator2!!
  
  //private GoogleSheetsPluginInputMeta meta;
  //private GoogleSheetsPluginInputData data;
  
  public GoogleSheetsPluginInput( TransformMeta transformMeta, GoogleSheetsPluginInputMeta meta, GoogleSheetsPluginInputData data, int copyNr, PipelineMeta pipelineMeta, Pipeline pipeline ) {
  ...
  /**
   ...
   public boolean init() {//StepMetaInterface smi, ITransformData sdi
		...
		//meta = (GoogleSheetsPluginInputMeta) smi;
        //data = (GoogleSheetsPluginInputData) sdi;
		...
		if (super.init()){
            try {				
				Sheets service = new Sheets.Builder(HTTP_TRANSPORT, JSON_FACTORY, GoogleSheetsPluginCredentials.getCredentialsJson(scope,resolve(meta.getJsonCredentialPath()))).setApplicationName(APPLICATION_NAME).build();
				String range=resolve(meta.getWorksheetId());
				ValueRange response = service.spreadsheets().values().get(resolve(meta.getSpreadsheetKey()),range).execute();             
				if(response==null) {
					log.logError("No data found for worksheet : "+resolve( meta.getWorksheetId())+" in spreadsheet :"+resolve( meta.getSpreadsheetKey()));
				    return false;
				} else
					{				
					...
					if (values == null || values.isEmpty()) {
						throw new HopTransformException("No response found for worksheet : "+resolve( meta.getWorksheetId())+" in spreadsheet :"+resolve( meta.getSpreadsheetKey()));
						} else {
							data.rows=values;
						}
					}			
  ...
  public boolean processRow() throws HopException {
    ...
    if (first) {
		...
		//data.outputRowMeta, getStepname(), null, getStepMeta(), this, repository, metaStore);
		data.currentRow++;
  ...
  private Object getRowDataValue(final IValueMeta targetValueMeta, final IValueMeta sourceValueMeta, final Object value, final DateFormat df) throws HopException
    {
        if (value == null) {
            return null;
        }
  ...
  /* private Object[] addRowData(Object[] r) throws HopException
    {
        // Parsing field
   ...
   private Object[] readRow() {
        try {
			...
			if (data.currentRow < data.rows.size()) {
				...
				if(outputIndex>row.size()-1){
				  ...
				  outputRowData[outputIndex++] = null;
						...
						if (value == null||value.toString().isEmpty()){
							...
							DateFormat df= (column.getType() == IValueMeta.TYPE_DATE)? new SimpleDateFormat(input.getFormat()): null;
                ...
                log.logBasic("Finished reading last row "+ Integer.toString(data.currentRow) +" / "+Integer.toString(data.rows.size()));
				return null;
            }
